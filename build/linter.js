(function (global, factory) {
	...
	(global = global || self, global.lint = factory());
	...
	function traverse(node, visitor) {
	  ...
	  var props = traverseProps[node.type];
	...
	var linter = (function (json, rules) {
	  var ast = parseJson(json);

	  if (ast) {
	    var errors = [];
	    rules.forEach(function (rule) {
	      traverse(ast, rule(function (error) {
	      ...
	      if (blockName === "text") {
	        ...
	        if (type === "h2") {
	          ...
	          headers.forEach(function (x) {
	            report(error(code$1, text$1, x.loc));
	          });
	          headers.length = 0;
	        }
	    ...
	    enter: function enter(node) {
	      ...
	      if (blockName === "text") {
	        ...
	        if (type === "h3") {
	          ...
	          headers.forEach(function (x) {
	            report(error(code$2, text$2, x.loc));
	          });
	          headers.length = 0;
	        }
	      } else if (blockName && !getProperty(node, "elem")) {
